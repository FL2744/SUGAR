...
class SettingsPage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        root.addWidget(
            page_header(
                ...
                "Secrets are held in memory for this app session and are never written by the Windows UI.",
            )
        )

        llm = Card(
            "LLM provider",
            "Used for translation, State triage, and agentic intelligence. Virginia Tech ARC uses its OpenAI-compatible endpoint.",
        )
        grid = QGridLayout()
        self.provider = EnumCombo(
            (("OpenAI", "openai"), ("Virginia Tech ARC", "arc"), ("Custom OpenAI-compatible", "custom"))
        )
        ...
        grid.addWidget(
            LabeledRow("API key", self.llm_key, "If blank, SUGAR_LLM_API_KEY from the environment is used."), 1, 1
        )
        ...
        sources = Card(
            "Source credentials",
            "Only use legitimate credentials or sessions you are authorized to use. SUGAR does not automate login or manufacture browser/session identities.",
        )
        ...
        source_grid.addWidget(
            LabeledRow(
                "Weibo session",
                self.weibo_cookie,
                "Optional. Anonymous public surfaces remain the default when this is blank.",
            ),
            2,
            0,
            1,
    ...
    def secrets(self) -> dict[str, str]:
        ...
        return {
            ...
            "bluesky_identifier": self.bluesky_id.text().strip()
            or os.environ.get("SUGAR_BLUESKY_IDENTIFIER", "").strip(),
...
class HomePage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        root.addWidget(
            page_header(
                "SUGAR Research Workbench",
                "Evidence-first collection, State Department research coding, macro/micro analytic intelligence, and reproducible briefing outputs.",
            )
        )
        ...
        text = QLabel(
            "Collect → Normalize → Triage → Human Verify → State Assessment → Intelligence → Red Team → Brief"
        )
        ...
        quick = Card(
            "Quick actions",
            "Use the full pages for advanced controls; these buttons jump directly to the relevant workflow.",
        )
        actions = QHBoxLayout()
        for label, target in (
            ("Collect data", "Collect"),
            ("Investigate Weibo", "Weibo"),
            ("Build State package", "State Workflow"),
            ("Run intelligence synthesis", "Intelligence"),
        ):
            button = QPushButton(label)
...
class CollectPage(QWidget):
    def __init__(self, run: Callable[[str, dict[str, Any], bool], None], settings: SettingsPage) -> None:
        ...
        root.addWidget(
            page_header(
                "Collection", "Run bounded interactive searches or durable, checkpointed high-volume harvest campaigns."
            )
        )
        tabs = QTabWidget()
    ...
    def _search_tab(self) -> QWidget:
        ...
        form.addWidget(
            LabeledRow(
                "Translate terms into",
                self.term_languages,
                "Example: Simplified Chinese, Russian. Leave blank to use only original terms.",
            ),
            2,
            0,
        )
        form.addWidget(
            LabeledRow("Post language filters", self.post_languages, "BCP-47 codes; currently most relevant to X."),
            2,
            1,
        )
    ...
    def _harvest_tab(self) -> QWidget:
        ...
        self.harvest_terms.setPlaceholderText(
            "One query per line. Use a reproducible research matrix rather than a single deep query."
        )
        ...
        form.addWidget(
            LabeledRow(
                "Unique-record target",
                self.harvest_target,
                "0 runs the entire bounded plan; otherwise collection stops after reaching the target.",
            ),
            3,
            0,
        )
        ...
        layout.addLayout(form)
        note = QLabel(
            "Rate limits remain authoritative. Harvest scales through durable sharding, checkpoint/resume, deduplication and backoff—not proxy/account rotation or access-control evasion."
        )
...
class WeiboPage(QWidget):
    def __init__(self, run: Callable[[str, dict[str, Any], bool], None], settings: SettingsPage) -> None:
        ...
        root.addWidget(
            page_header(
                "Weibo Research",
                "Investigate real public posts at micro level or qualify a repeatable campaign against explicit State-facing research thresholds.",
            )
        )
        tabs = QTabWidget()
    ...
    def _investigate_tab(self) -> QWidget:
        ...
        note = QLabel(
            "Every surface is reported independently. Access-limited repost/timeline surfaces are not converted into observed zeroes."
        )
        note.setObjectName("hint")
        note.setWordWrap(True)
        layout.addWidget(note)
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Investigate Post", self._run_investigation))
        layout.addLayout(row)
        layout.addStretch(1)
        return page
    ...
    def _qualification_tab(self) -> QWidget:
        ...
        self.qual_delay = QDoubleSpinBox()
        self.qual_delay.setRange(0, 60)
        self.qual_delay.setValue(1.0)
        self.qual_delay.setSuffix(" s")
        ...
        self.qual_output = PathField(mode="directory")
        self.qual_output.setText(self.settings.default_output())
        grid = QGridLayout()
        ...
        grid.addWidget(
            LabeledRow(
                "Completed human audit CSV",
                self.qual_audit_file,
                "Optional on first run. A deterministic audit sample is generated for labeling.",
            ),
            3,
            1,
        )
        ...
        layout.addLayout(grid)
        note = QLabel(
            "Qualification runs the complete bounded query plan. It measures coverage, failures, access limits, provenance, duplicates, repeatability and real-post/comment access; it is not an official State certification or ATO."
        )
        note.setObjectName("hint")
        note.setWordWrap(True)
        layout.addWidget(note)
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Run Qualification", self._run_qualification))
        layout.addLayout(row)
        layout.addStretch(1)
        return page
...
class StatePage(QWidget):
    def __init__(self, run: Callable[[str, dict[str, Any], bool], None], settings: SettingsPage) -> None:
        ...
        root.addWidget(
            page_header(
                "State Department Workflow",
                "Turn ResearchObservations into auditable assessments, human review artifacts, American Spaces/EducationUSA comparisons, maps, networks and briefing outputs.",
            )
        )
        tabs = QTabWidget()
    ...
    def _package_tab(self) -> QWidget:
        ...
        self.state_output = PathField(mode="directory")
        self.state_output.setText(self.settings.default_output())
        ...
        grid.addWidget(
            LabeledRow("State assessments", self.state_assess, "Optional. Blank assessments are created when omitted."),
            1,
            0,
        )
        ...
        layout.addLayout(grid)
        note = QLabel(
            "The package defaults to verified-only State-facing outputs and keeps attendance/views/likes/comments/shares separate rather than manufacturing an influence score."
        )
        note.setObjectName("hint")
        note.setWordWrap(True)
        layout.addWidget(note)
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Build Complete State Package", self._run_package))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _run_package(self) -> None:
        if not self.state_obs.text():
            QMessageBox.warning(self, "Missing observations", "Choose a ResearchObservation dataset.")
            return
        self.run_operation(
            "state-package",
            {
                "observations": self.state_obs.text(),
                "assessments": self.state_assess.text(),
                "us_sites": self.state_us.text(),
                "entities": self.state_entities.text(),
                "previous_assessments": self.state_previous.text(),
                "output_directory": self.state_output.text() or self.settings.default_output(),
                "name": self.state_name.text().strip() or "state_research",
                "title": self.state_title.text().strip(),
                "current_start": self.current_start.text().strip() or "2024-01-01",
    ...
    def _triage_tab(self) -> QWidget:
        ...
        self.triage_limit = NumberField(0, 1_000_000, 0)
        self.triage_limit.setSpecialValueText("All observations")
        ...
        layout.addWidget(
            LabeledRow(
                "Limit",
                self.triage_limit,
                "0 processes the entire dataset. AI suggestions remain unverified until an analyst reviews them.",
            )
        )
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Run Guarded AI Triage", self._run_triage))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _run_triage(self) -> None:
        if not self.triage_obs.text():
            QMessageBox.warning(self, "Missing observations", "Choose a ResearchObservation dataset.")
            return
        ...
        self.run_operation(
            "state-triage",
            {
                "observations": self.triage_obs.text(),
                "output_file": target,
                "limit": self.triage_limit.value() or None,
                "llm": self.settings.llm_config(),
            },
            True,
        )
    ...
    def _review_tab(self) -> QWidget:
        ...
        layout.setContentsMargins(16, 16, 16, 16)
        export = Card(
            "Export analyst review workbook",
            "Creates an Excel workbook containing assessment and claim decisions without allowing AI to self-verify them.",
        )
        self.review_obs = PathField(mode="file", extensions=("csv", "xlsx", "jsonl"))
        self.review_assess = PathField(mode="file", extensions=("jsonl",))
        self.review_book = PathField(mode="save", save_extension="xlsx")
        export.layout.addWidget(LabeledRow("Observations", self.review_obs))
        export.layout.addWidget(LabeledRow("Assessments", self.review_assess))
        export.layout.addWidget(LabeledRow("Review workbook", self.review_book))
        export.layout.addWidget(primary_button("Export Review Workbook", self._review_export))
        layout.addWidget(export)
        apply = Card(
            "Apply analyst decisions",
            "Imports reviewed decisions back through the same evidence and verification constraints.",
        )
        self.apply_assess = PathField(mode="file", extensions=("jsonl",))
        self.apply_book = PathField(mode="file", extensions=("xlsx",))
        self.apply_out = PathField(mode="save", save_extension="jsonl")
        apply.layout.addWidget(LabeledRow("Original assessments", self.apply_assess))
        apply.layout.addWidget(LabeledRow("Completed review workbook", self.apply_book))
        apply.layout.addWidget(LabeledRow("Reviewed assessments", self.apply_out))
        apply.layout.addWidget(primary_button("Apply Human Review", self._review_apply))
        layout.addWidget(apply)
        layout.addStretch(1)
        return page

    def _review_export(self) -> None:
        if not self.review_obs.text() or not self.review_assess.text():
            QMessageBox.warning(self, "Missing files", "Choose observations and assessments.")
            return
        out = self.review_book.text() or str(Path(self.settings.default_output()) / "state_review.xlsx")
        self.run_operation(
            "state-review-export",
            {"observations": self.review_obs.text(), "assessments": self.review_assess.text(), "output_file": out},
            False,
        )

    def _review_apply(self) -> None:
        if not self.apply_assess.text() or not self.apply_book.text():
            QMessageBox.warning(self, "Missing files", "Choose assessments and a completed review workbook.")
            return
        out = self.apply_out.text() or str(Path(self.settings.default_output()) / "state_reviewed.jsonl")
        self.run_operation(
            "state-review-apply",
            {"assessments": self.apply_assess.text(), "workbook": self.apply_book.text(), "output_file": out},
            False,
        )
    ...
    def _audit_tab(self) -> QWidget:
        ...
        self.audit_obs = PathField(mode="file", extensions=("csv", "xlsx", "jsonl"))
        self.audit_assess = PathField(mode="file", extensions=("jsonl",))
        self.audit_out = PathField(mode="save", save_extension="json")
        audit.layout.addWidget(LabeledRow("Observations", self.audit_obs))
        audit.layout.addWidget(LabeledRow("Assessments", self.audit_assess))
        audit.layout.addWidget(LabeledRow("Audit JSON", self.audit_out))
        audit.layout.addWidget(primary_button("Run Audit", self._audit))
        layout.addWidget(audit)
        diff = Card("Assessment snapshot diff")
        self.diff_prev = PathField(mode="file", extensions=("jsonl",))
        self.diff_cur = PathField(mode="file", extensions=("jsonl",))
        self.diff_out = PathField(mode="save", save_extension="json")
        diff.layout.addWidget(LabeledRow("Previous assessments", self.diff_prev))
        diff.layout.addWidget(LabeledRow("Current assessments", self.diff_cur))
        diff.layout.addWidget(LabeledRow("Diff JSON", self.diff_out))
        diff.layout.addWidget(primary_button("Compare Snapshots", self._diff))
        layout.addWidget(diff)
        layout.addStretch(1)
        return page

    def _audit(self) -> None:
        if not self.audit_obs.text() or not self.audit_assess.text():
            QMessageBox.warning(self, "Missing files", "Choose observations and assessments.")
            return
        out = self.audit_out.text() or str(Path(self.settings.default_output()) / "state_audit.json")
        self.run_operation(
            "state-audit",
            {"observations": self.audit_obs.text(), "assessments": self.audit_assess.text(), "output_file": out},
            False,
        )

    def _diff(self) -> None:
        if not self.diff_prev.text() or not self.diff_cur.text():
            QMessageBox.warning(self, "Missing files", "Choose both assessment snapshots.")
            return
        out = self.diff_out.text() or str(Path(self.settings.default_output()) / "state_diff.json")
        self.run_operation(
            "state-diff",
            {"previous": self.diff_prev.text(), "current": self.diff_cur.text(), "output_file": out},
            False,
        )
    ...
    def _templates_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        us = Card("U.S. public-diplomacy presence template")
        self.us_template = PathField(mode="save", save_extension="csv")
        us.layout.addWidget(LabeledRow("Output CSV", self.us_template))
        us.layout.addWidget(
            primary_button(
                "Create U.S. Sites Template",
                lambda: self._template("state-template-us-sites", self.us_template, "us_presence.csv"),
            )
        )
        layout.addWidget(us)
        ent = Card("Monitored entity / alias registry")
        self.entity_template = PathField(mode="save", save_extension="csv")
        ent.layout.addWidget(LabeledRow("Output CSV", self.entity_template))
        ent.layout.addWidget(
            primary_button(
                "Create Entity Template",
                lambda: self._template("state-template-entities", self.entity_template, "monitored_entities.csv"),
            )
        )
        layout.addWidget(ent)
        plan = Card("Generate watch-query plan from entity registry")
        self.plan_entities = PathField(mode="file", extensions=("csv", "xlsx"))
        self.plan_out = PathField(mode="save", save_extension="txt")
        plan.layout.addWidget(LabeledRow("Entity registry", self.plan_entities))
        plan.layout.addWidget(LabeledRow("Query plan", self.plan_out))
        plan.layout.addWidget(primary_button("Generate Query Plan", self._query_plan))
        layout.addWidget(plan)
        layout.addStretch(1)
        return page

    def _template(self, operation: str, field: PathField, name: str) -> None:
        out = field.text() or str(Path(self.settings.default_output()) / name)
        self.run_operation(operation, {"output_file": out}, False)

    def _query_plan(self) -> None:
        if not self.plan_entities.text():
            QMessageBox.warning(self, "Missing registry", "Choose an entity registry.")
            return
        out = self.plan_out.text() or str(Path(self.settings.default_output()) / "query_plan.txt")
        self.run_operation("state-query-plan", {"entities": self.plan_entities.text(), "output_file": out}, False)
...
class IntelligencePage(QWidget):
    def __init__(self, run: Callable[[str, dict[str, Any], bool], None], settings: SettingsPage) -> None:
        ...
        root.addWidget(
            page_header(
                ...
                "Measure the corpus first, audit epistemic debt, then run bounded specialist agents, evidence-neighborhood refinement, integration, red-team critique and longitudinal comparison.",
            )
        )
        tabs = QTabWidget()
        tabs.addTab(self._deterministic_tab(), "Packet & Tradecraft")
        tabs.addTab(self._synthesis_tab(), "Agentic Synthesis")
        tabs.addTab(self._hypothesis_tab(), "Competing Hypotheses")
        tabs.addTab(self._compare_tab(), "Longitudinal")
        root.addWidget(tabs, 1)

    def _base_inputs(self):
        obs = PathField(mode="file", extensions=("csv", "xlsx", "jsonl"))
        assessments = PathField(mode="file", extensions=("jsonl",))
        return obs, assessments

    def _deterministic_tab(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        self.intel_obs, self.intel_assess = self._base_inputs()
        self.intel_country = QLineEdit()
        self.intel_case = QLineEdit()
        self.case_limit = NumberField(1, 500, 20)
        self.packet_out = PathField(mode="save", save_extension="json")
        self.trade_out = PathField(mode="save", save_extension="json")
        grid = QGridLayout()
        grid.addWidget(LabeledRow("Observations", self.intel_obs), 0, 0)
        grid.addWidget(LabeledRow("State assessments", self.intel_assess), 0, 1)
        grid.addWidget(LabeledRow("Country scope", self.intel_country, "Optional"), 1, 0)
        grid.addWidget(
            LabeledRow("Observation ID", self.intel_case, "Optional micro-case scope; takes precedence over country."),
            1,
            1,
        )
        grid.addWidget(LabeledRow("Representative case limit", self.case_limit), 2, 0)
        grid.addWidget(LabeledRow("Packet output", self.packet_out), 2, 1)
        layout.addLayout(grid)
        row = QHBoxLayout()
        row.addWidget(primary_button("Build Intelligence Packet", self._packet))
        row.addWidget(QPushButton("Run Tradecraft Audit", clicked=self._tradecraft))
        row.addStretch(1)
        layout.addLayout(row)
        layout.addWidget(LabeledRow("Tradecraft audit output", self.trade_out))
        note = QLabel(
            "These operations require no LLM. Use them to inspect concentration/diversity, comparability, source adequacy, analytic tensions and epistemic debt before synthesis."
        )
        note.setObjectName("hint")
        note.setWordWrap(True)
        layout.addWidget(note)
    ...
    def _require_base(self, obs: PathField, assess: PathField) -> bool:
        if not obs.text() or not assess.text():
            QMessageBox.warning(self, "Missing files", "Choose observations and State assessments.")
            return False
        return True

    def _packet(self) -> None:
        if not self._require_base(self.intel_obs, self.intel_assess):
            return
        out = self.packet_out.text() or str(Path(self.settings.default_output()) / "intelligence_packet.json")
        self.run_operation(
            "intel-packet",
            {
                "observations": self.intel_obs.text(),
                "assessments": self.intel_assess.text(),
                "output_file": out,
                "country": self.intel_country.text().strip(),
                "observation_id": self.intel_case.text().strip(),
                "case_limit": self.case_limit.value(),
            },
            False,
        )
    ...
    def _tradecraft(self) -> None:
        if not self._require_base(self.intel_obs, self.intel_assess):
            return
        out = self.trade_out.text() or str(Path(self.settings.default_output()) / "tradecraft_audit.json")
        self.run_operation(
            "intel-tradecraft",
            {"observations": self.intel_obs.text(), "assessments": self.intel_assess.text(), "output_file": out},
            False,
        )
    ...
    def _synthesis_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        self.syn_obs, self.syn_assess = self._base_inputs()
        self.syn_country = QLineEdit()
        self.syn_case = QLineEdit()
        self.syn_depth = EnumCombo((("Quick", "quick"), ("Standard", "standard"), ("Deep iterative", "deep")))
        self.syn_depth.setCurrentIndex(1)
        self.syn_workers = NumberField(1, 16, 4)
        self.syn_name = QLineEdit("analytic_intelligence")
        self.syn_output = PathField(mode="directory")
        self.syn_output.setText(self.settings.default_output())
        grid = QGridLayout()
        grid.addWidget(LabeledRow("Observations", self.syn_obs), 0, 0)
        grid.addWidget(LabeledRow("State assessments", self.syn_assess), 0, 1)
        grid.addWidget(LabeledRow("Country scope", self.syn_country, "Optional"), 1, 0)
        grid.addWidget(LabeledRow("Observation ID", self.syn_case, "Optional micro-case scope"), 1, 1)
        grid.addWidget(LabeledRow("Depth", self.syn_depth), 2, 0)
        grid.addWidget(LabeledRow("Parallel workers", self.syn_workers), 2, 1)
        grid.addWidget(LabeledRow("Synthesis name", self.syn_name), 3, 0)
        grid.addWidget(LabeledRow("Output folder", self.syn_output), 3, 1)
        layout.addLayout(grid)
        note = QLabel(
            "Deep mode runs first-pass specialists, retrieves cited and comparable evidence, forces specialist reassessment, then integrates, red-teams, and revises. Multiple agents are functional decomposition—not independent corroboration."
        )
        note.setObjectName("hint")
        note.setWordWrap(True)
        layout.addWidget(note)
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Run Synthesis", self._synthesize))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _synthesize(self) -> None:
        if not self._require_base(self.syn_obs, self.syn_assess):
            return
        self.run_operation(
            "intel-synthesize",
            {
                "observations": self.syn_obs.text(),
                "assessments": self.syn_assess.text(),
                "output_directory": self.syn_output.text() or self.settings.default_output(),
                "name": self.syn_name.text().strip() or "analytic_intelligence",
                "country": self.syn_country.text().strip(),
                "observation_id": self.syn_case.text().strip(),
                "depth": self.syn_depth.value(),
                "workers": self.syn_workers.value(),
                "llm": self.settings.llm_config(),
            },
            True,
        )
    ...
    def _hypothesis_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        self.hyp_syn = PathField(mode="file", extensions=("json",))
        self.hyp_output = PathField(mode="directory")
        self.hyp_output.setText(self.settings.default_output())
        self.hyp_name = QLineEdit("analytic_intelligence")
        layout.addWidget(LabeledRow("Synthesis JSON", self.hyp_syn))
        layout.addWidget(LabeledRow("Output folder", self.hyp_output))
        layout.addWidget(LabeledRow("Output name", self.hyp_name))
        note = QLabel(
            "Creates a support/contradiction/discrimination matrix and a least-inconsistent ordering. It does not numerically score truth or convert missing collection into negative evidence."
        )
        note.setObjectName("hint")
        note.setWordWrap(True)
        layout.addWidget(note)
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Build Hypothesis Matrix", self._hypotheses))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _hypotheses(self) -> None:
        if not self.hyp_syn.text():
            QMessageBox.warning(self, "Missing synthesis", "Choose a synthesis JSON file.")
            return
        self.run_operation(
            "intel-hypotheses",
            {
                "synthesis": self.hyp_syn.text(),
                "output_directory": self.hyp_output.text() or self.settings.default_output(),
                "name": self.hyp_name.text().strip() or "analytic_intelligence",
            },
            False,
        )
    ...
    def _compare_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        self.comp_prev = PathField(mode="file", extensions=("json",))
        self.comp_cur = PathField(mode="file", extensions=("json",))
        self.comp_kind = EnumCombo((("Agentic synthesis", "synthesis"), ("Deterministic packet", "packet")))
        self.comp_out = PathField(mode="save", save_extension="json")
        layout.addWidget(LabeledRow("Previous reporting cycle", self.comp_prev))
        layout.addWidget(LabeledRow("Current reporting cycle", self.comp_cur))
        layout.addWidget(LabeledRow("Artifact type", self.comp_kind))
        layout.addWidget(LabeledRow("Comparison output", self.comp_out))
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Compare Reporting Cycles", self._compare))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _compare(self) -> None:
        if not self.comp_prev.text() or not self.comp_cur.text():
            QMessageBox.warning(self, "Missing files", "Choose previous and current intelligence artifacts.")
            return
        out = self.comp_out.text() or str(Path(self.settings.default_output()) / "intelligence_comparison.json")
        self.run_operation(
            "intel-compare",
            {
                "previous": self.comp_prev.text(),
                "current": self.comp_cur.text(),
                "kind": self.comp_kind.value(),
                "output_file": out,
            },
            False,
        )
...
class ReportsPage(QWidget):
    def __init__(self, run: Callable[[str, dict[str, Any], bool], None], settings: SettingsPage) -> None:
        ...
        root.addWidget(
            page_header(
                "Maps & Reports", "Create standalone maps and Word/PDF reports from existing normalized result files."
            )
        )
        tabs = QTabWidget()
    ...
    def _map_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        self.map_source = PathField(mode="file", extensions=("csv", "xlsx"))
        self.map_out = PathField(mode="save", save_extension="html")
        layout.addWidget(LabeledRow("Existing results", self.map_source))
        layout.addWidget(LabeledRow("HTML map", self.map_out))
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Create Map", self._map))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _map(self) -> None:
        if not self.map_source.text():
            QMessageBox.warning(self, "Missing file", "Choose a normalized results file.")
            return
        out = self.map_out.text() or str(
            Path(self.map_source.text()).with_name(Path(self.map_source.text()).stem + "_map.html")
        )
        self.run_operation("map", {"source_file": self.map_source.text(), "output_file": out}, False)

    def _report_tab(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(16, 16, 16, 16)
        self.report_source = PathField(mode="file", extensions=("csv", "xlsx"))
        self.report_stem = PathField(mode="save", save_extension="pdf")
        self.report_format = EnumCombo((("PDF", "pdf"), ("Word", "docx"), ("PDF + Word", "both")))
        layout.addWidget(LabeledRow("Existing results", self.report_source))
        layout.addWidget(LabeledRow("Output path", self.report_stem, "Extension is ignored for combined output."))
        layout.addWidget(LabeledRow("Format", self.report_format))
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(primary_button("Create Report", self._report))
        layout.addLayout(row)
        layout.addStretch(1)
        return page

    def _report(self) -> None:
        if not self.report_source.text():
            QMessageBox.warning(self, "Missing file", "Choose a normalized results file.")
            return
        raw = self.report_stem.text() or str(
            Path(self.report_source.text()).with_name(Path(self.report_source.text()).stem + "_analysis")
        )
        stem = str(Path(raw).with_suffix(""))
        self.run_operation(
            "analysis",
            {
                "source_file": self.report_source.text(),
                "output_stem": stem,
                "output_format": self.report_format.value(),
            },
            False,
        )
...
class ActivityDock(QDockWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__("Activity & Outputs", parent)
        ...
        self.copy = QPushButton("Copy log")
        ...
        self.output_host = QWidget()
        self.output_layout = QHBoxLayout(self.output_host)
        self.output_layout.setContentsMargins(0, 0, 0, 0)
        self.output_layout.addWidget(QLabel("Outputs:"))
        self.output_layout.addStretch(1)
        root.addWidget(self.output_host)
    ...
    def append_event(self, payload: dict[str, Any]) -> None:
        ...
        if event in {"backend", "diagnostics"}:
            return
        if event == "complete":
            ...
            message = f"backend: {payload.get('message', '')}"
        else:
            details = []
            for key in (
                "operation",
        ...
        if isinstance(payload.get("current"), int) and isinstance(payload.get("total"), int):
            self.set_progress(int(payload["current"]), int(payload["total"]))
        elif isinstance(payload.get("index"), int) and isinstance(payload.get("total"), int):
            self.set_progress(int(payload["index"]), int(payload["total"]))
    ...
    def set_outputs(self, paths: list[str]) -> None:
        ...
        for path in paths[:12]:
            self.output_layout.addWidget(OutputChip(path))
        if len(paths) > 12:
            self.output_layout.addWidget(QLabel(f"+{len(paths) - 12} more"))
        self.output_layout.addStretch(1)
...
class MainWindow(QMainWindow):
    def __init__(self, *, smoke: bool = False) -> None:
        super().__init__()
        self.setWindowTitle("SUGAR — State Research Workbench")
        self.resize(1380, 900)
        self.setMinimumSize(1080, 720)
        icon = resource_path("sugar-logo.png")
        if icon.is_file():
        ...
        side.setContentsMargins(8, 16, 8, 12)
        brand = QLabel("SUGAR")
        brand.setObjectName("brand")
        sub = QLabel("State Research Workbench")
        ...
        version = QLabel("Evidence-first OSINT + analysis")
        ...
        page_defs = [
            ("Home", self.home),
            ("Collect", CollectPage(self.run_operation, self.settings_page)),
            ("Weibo", WeiboPage(self.run_operation, self.settings_page)),
            ("State Workflow", StatePage(self.run_operation, self.settings_page)),
            ("Intelligence", IntelligencePage(self.run_operation, self.settings_page)),
            ("Maps & Reports", ReportsPage(self.run_operation, self.settings_page)),
        ...
        self.activity = ActivityDock(self)
        self.addDockWidget(Qt.BottomDockWidgetArea, self.activity)
        self.activity.cancel_requested.connect(self.runner.cancel)
        self.runner.event.connect(self._event)
        self.runner.outputs_changed.connect(self.activity.set_outputs)
        self.runner.error.connect(self._error)
        self.runner.running_changed.connect(self.activity.set_running)
        self.settings_page.diagnostics_requested.connect(self._diagnostics_run)
        self._build_menu()
        if not smoke:
    ...
    def _build_menu(self) -> None:
        ...
        open_output = QAction("Open default output folder", self)
        open_output.triggered.connect(
            lambda: QDesktopServices.openUrl(QUrl.fromLocalFile(self.settings_page.default_output()))
        )
        ...
        diag = QAction("Backend diagnostics", self)
        diag.triggered.connect(self._diagnostics_run)
        tools.addAction(diag)
        cancel = QAction("Cancel current operation", self)
        cancel.triggered.connect(self.runner.cancel)
        tools.addAction(cancel)

    def navigate(self, name: str) -> None:
        if name in self.pages:
            self.nav.setCurrentRow(self.pages[name])

    def run_operation(self, command: str, config: dict[str, Any], requires_llm: bool = False) -> None:
        if self.runner.is_running:
            QMessageBox.information(
                self, "SUGAR is busy", "Cancel or finish the current operation before starting another."
            )
            return
        ...
        if requires_llm and not secrets.get("llm_api_key"):
            QMessageBox.warning(
                self,
                "LLM key required",
                "This operation needs an LLM API key. Add one in Settings or set SUGAR_LLM_API_KEY in the environment.",
            )
            self.navigate("Settings")
            return
        if "llm" not in config and requires_llm:
            config["llm"] = self.settings_page.llm_config()
        try:
    ...
    def closeEvent(self, event) -> None:
        if self.runner.is_running:
            choice = QMessageBox.question(
                self,
                "Operation running",
                "A SUGAR operation is still running. Cancel it and exit?",
