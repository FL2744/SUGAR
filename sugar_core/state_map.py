    ...
    *,
    ...
    for obs, assessment, location in mapped:
        ...
        folium.CircleMarker(
            ...
            popup=folium.Popup(_popup(obs, assessment, location), max_width=520),
        ).add_to(cluster)
    ...
    legend = f"""
    <div style="position: fixed; bottom: 20px; left: 20px; z-index: 9999; background: white; border: 1px solid #888; padding: 10px; max-width: 390px; font-size: 12px;">
      ...
      <b>Analytic guardrail:</b> density is activity-location density, not influence, and is never weighted by likes, views, or attendance.
    </div>
    """
