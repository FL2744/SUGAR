...
def _object_payload(value: object, operation: str) -> dict:
    if not isinstance(value, dict):
        raise RuntimeError(f"{operation} returned an unexpected JSON object shape.")
    return value
...
def collect_x(
    *,
    ...
    since: str | None = None,
    until: str | None = None,
    post_languages: list[str] | None = None,
    ...
    if max_posts_per_query < 1 or max_pages_per_query < 1:
        raise ValueError("X collection limits must be at least 1.")
    ...
    for original_query in search_terms:
        ...
        if not query:
            continue
        if post_languages and "lang:" not in query:
        ...
        for _ in range(max_pages_per_query):
            ...
            response.raise_for_status()
            payload = _object_payload(response.json(), "X search")
            ...
            for item in page_items:
                ...
                refs = [ref for ref in (item.get("referenced_tweets") or []) if isinstance(ref, dict)]
                repost = any(ref.get("type") == "retweeted" for ref in refs)
                ...
                url = (
                    f"https://x.com/{handle}/status/{native_id}"
                    if handle
...
def collect_bluesky(
    *,
    ...
    session = session or create_session()
    if max_posts_per_query < 1 or max_pages_per_query < 1:
        raise ValueError("Bluesky collection limits must be at least 1.")
    records: OrderedDict[tuple[str, str], PostRecord] = OrderedDict()
    if access_jwt:
...
def collect_mastodon(
    *,
    ...
    session = session or create_session()
    if max_posts_per_query < 1 or max_pages_per_query < 1:
        raise ValueError("Mastodon collection limits must be at least 1.")
    instance_url = instance_url.rstrip("/")
    records: OrderedDict[tuple[str, str], PostRecord] = OrderedDict()
    endpoint = f"{instance_url}/api/v2/search"
        ...
        for _ in range(max_pages_per_query):
            ...
            for status in statuses:
                if not isinstance(status, dict):
                    continue
                repost = bool(status.get("reblog"))
                content = status.get("reblog") if isinstance(status.get("reblog"), dict) else status
                if repost and not include_reposts:
                    continue
                ...
                raw = {
                    "reply_count": content.get("replies_count", 0),
                    "reblog_count": content.get("reblogs_count", 0),
