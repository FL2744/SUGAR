...
def save_records(
    records: Iterable[PostRecord], output_file: str | Path, *, metadata: dict | None = None
) -> pd.DataFrame:
    ...
    with atomic_path(csv_path) as temporary_csv:
        df.to_csv(temporary_csv, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL, lineterminator="\n")
    with atomic_path(xlsx_path) as temporary_xlsx:
        with pd.ExcelWriter(temporary_xlsx, engine="openpyxl") as writer:
            ...
            for idx, name in enumerate(df.columns, 1):
                width = 18
                if name in {"original_text", "translated_text", "translated_en", "location_reason"}:
                    width = 60
                elif name in {"canonical_url", "source_url", "post_url", "x_url"}:
                    width = 45
