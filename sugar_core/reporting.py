...
def _prepare(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    if df.empty:
        raise ValueError("The selected results file contains no records.")
    work = df.copy()
    ...
    work["language"] = (
        work.get(lang_col, pd.Series(["unknown"] * len(work), index=work.index))
        .fillna("unknown")
        .replace("", "unknown")
    )
    ...
    return work, {
        ...
        "engagement": int(work.engagement_total.sum()),
        "impressions": int(work.impressions.sum()),
...
def _terms(work: pd.DataFrame) -> list[tuple[str, int]]:
    ...
    if not text.strip():
        text = " ".join(work.get("original_text", pd.Series(dtype=str)).fillna("").astype(str))
    stop = {
        "the",
        "and",
        "that",
...
def _docx(work: pd.DataFrame, metrics: dict, charts: dict[str, Path], source: str, output: str) -> None:
    ...
    for i, h in enumerate(("Measure", "Value")):
        table.rows[0].cells[i].text = h
    for key, value in [
        ("Records", f"{metrics['records']:,}"),
        ("Unique IDs", f"{metrics['unique_ids']:,}"),
        ("Observed dates", f"{metrics['start']} to {metrics['end']}"),
        ("Recorded engagement", f"{metrics['engagement']:,}"),
    ...
    doc.add_picture(str(charts["platform"]), width=Inches(6.5))
    doc.add_picture(str(charts["language"]), width=Inches(6.5))
    doc.add_heading("Geographic distribution", 1)
    doc.add_picture(str(charts["location"]), width=Inches(6.5))
    doc.add_paragraph(
    ...
    for platform, value in platform_eng.items():
        doc.add_paragraph(f"{platform}: {int(value):,} recorded interactions", style="List Bullet")
...
def _pdf(work: pd.DataFrame, metrics: dict, charts: dict[str, Path], source: str, output: str) -> None:
    ...
    story += [t, Spacer(1, 12)]
    for title, key in (
        ("Collection composition", "platform"),
        ("Languages", "language"),
        ("Geographic distribution", "location"),
    ):
        story += [
            Paragraph(title, styles["Heading1"]),
            Image(str(charts[key]), width=6.3 * inch, height=2.8 * inch),
            Spacer(1, 8),
        ]
    story += [
        PageBreak(),
        Paragraph("Method caveats", styles["Heading1"]),
        Paragraph(
            "Counts describe retrieved records, not the full population of online discussion. Platform search coverage, pagination, source availability, and query design affect what is observed. Locations require source review and human verification before geographic claims.",
            styles["BodyText"],
        ),
        Spacer(1, 8),
        Paragraph(f"Source file: {Path(source).name}", styles["BodyText"]),
    ]
    SimpleDocTemplate(
        output,
        pagesize=letter,
...
def create_analysis_report(source_file: str, output_stem: str, output_format: str = "both") -> list[str]:
    ...
    work, metrics = _prepare(load_results(source_file))
    base = Path(output_stem).expanduser().resolve()
    base.parent.mkdir(parents=True, exist_ok=True)
    outputs: list[str] = []
    with tempfile.TemporaryDirectory(prefix="sugar-report-") as tmp:
        tmp = Path(tmp)
        charts = {k: tmp / f"{k}.png" for k in ("platform", "language", "location")}
