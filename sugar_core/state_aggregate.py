...
def build_state_rollups(
    ...
    for (country, city), rows in sorted(grouped.items()):
        ...
        verified = [(obs, assessment) for obs, assessment in rows if assessment.brief_eligible and obs.verification_state == "human_verified"]
    ...
    return {
        ...
        "overall": _rollup_row("ALL", "", [(observation_map[a.observation_id], a) for a in assessments if a.observation_id in observation_map], overall_verified),
        "countries": countries,
        "places": places,
    }
...
def _rollup_row(
    ...
    return {
        ...
        "program_domains": _sorted_counts(domain for _, assessment in verified for domain in assessment.program_domains),
        "strategic_audiences": _sorted_counts(audience for _, assessment in verified for audience in assessment.strategic_audiences),
    ...
    *,
    ...
    for path, rows in ((country_path, payload["countries"]), (place_path, payload["places"])):
        with path.open("w", encoding="utf-8-sig", newline="") as stream:
            ...
            for row in rows:
                raw = dict(row)
                for key in ("program_domains", "strategic_audiences", "narrative_tags", "observation_types", "review_states"):
