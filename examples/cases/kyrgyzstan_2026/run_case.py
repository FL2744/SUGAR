...
def _case_findings(observations, assessments, sites) -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                "EducationUSA Kyrgyzstan is fully online from April 1, 2026 and is represented as a non-spatial U.S. service. "
                "The current overlap model derives service overlap from the nearest physical U.S. site, so nationally available virtual advising is not automatically counted."
            ),
        ...
        },
        {
            ...
            "description": (
                "The current U.S.-presence schema does not store per-site location precision/uncertainty. This reproducible case therefore uses city-centroid coordinates for physical American Spaces and passes a conservative 12 km site uncertainty envelope to the map."
            ),
            "recommended_fix": "Add per-site location precision, confidence, and uncertainty fields and let State proximity consume them directly.",
        },
        {
...
def run_case(output_root: Path, *, clean: bool = False) -> dict:
    ...
    sources_path = _write_json(references_dir / "sources.json", sources_manifest(observations, sites))
    ...
    findings = _case_findings(observations, assessments, sites)
    ...
    preliminary = [
        ...
        "",
        "The U.S. comparison layer contains eight physical American Spaces represented conservatively at city precision plus EducationUSA Kyrgyzstan as a non-spatial national virtual service. The analyst map is intentionally available before human verification; the verified-only map contains no PRC observations until review gates are satisfied.",
        "",
    ...
    # The core purpose of this case is to exercise fail-closed behavior on real public data.
