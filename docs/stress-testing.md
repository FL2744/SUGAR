...
Use a supported Python 3.11–3.14 runtime. From the repository root:
...
The local Windows/Python 3.14 storage-only qualification on 2026-09-19 used 1,000,000 rows with a
10,000-row in-memory sample. SQLite upsert took 134.529 s, the bounded checkpoint read took
8.133 s, sample frame construction took 3.831 s, and the SQLite artifact was 1,394,016,256 bytes
(1.30 GiB). The run stayed within its 180 s operation and 5 GiB disk limits. Python allocation
tracing was disabled, so this run gives no peak-allocation measurement. These are host observations,
not release thresholds; repeat them on representative deployment hardware before setting budgets.
